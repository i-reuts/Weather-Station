...
public class WeatherStationActivity extends Activity {
	...
	private Switch mHumidexUnitSwitch;
	...
	private boolean mHumidexOn = false;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mHumidexUnitSwitch = (Switch) findViewById(R.id.humi_unit_switch);
		...
		mHumidexUnitSwitch.setOnCheckedChangeListener(new OnCheckedChangeListener() {

			/**
			 * Called when the Humidex switch is clicked, causing the state to
			 * change. Saves the Temperature/Humidex setting and updates the display to show the
			 * desired unit.
			 * 
			 * @param buttonView The mTemperatureUnitSwitch switch object
			 * @param isChecked  Whether the switch is in the checked (Humidex) or unchecked
			 * (Temperature) state.
			 */
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				// Save the unit so we know what to use when a new measurement comes in.
				mHumidexOn = isChecked;
	...
	private void updateDisplayedUnits() {
		// Update the unit text for the humidex/temperature (desired unit = mHumidexOn)
		if (mHumidexOn)
			mTemperatureHeaderView.setText(getString(R.string.humidex_title));
		else
		...
		double displayTemp = convertHumidex(mLastTemperature, mLastHumidity);
	...
	/**
	 * Convert the application to display either temperature or humidex set by the loaded settings (mHumidexOn
	 * member variable).
	 * 
	 * @param celsius
	 *            The temperature value in Celsius.
	 * @return The converted temperature value.
	 */
	private double convertHumidex(double celsius, double relativeHumidity) {
		if(mHumidexOn) {
			double a = 6.122e-3;
			double b = 17.67;
			double c = 243.5;
		
			double y = Math.log(relativeHumidity/100) + (b * celsius)/(c + celsius);
			
			double Tdp = (c * y)/(b - y) + 273.15;
			
			double humidex = celsius + 0.5555 * (6.11 * Math.exp(5417.7530 * (1/273.16 - 1/Tdp)) - 10);
	...
	public class ManagerListener extends SensorTagLoggerListener implements SensorTagListener {
		...
		public void onUpdateAmbientTemperature(SensorTagManager mgr, double temp) {
			...
			double displayTemp = convertHumidex(temp, mLastHumidity);
			displayTemp = convertTemperatureUnit(displayTemp);
